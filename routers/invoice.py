...
def create_new_invoice(invoice: InvoiceGeneration, db: Session = Depends(db_session)):
    try:
...
def get_invoices_per_user(user_id: int, db: Session = Depends(db_session)):
    try:
...
def get_invoices_per_business(business_id: int, db: Session = Depends(db_session)):
    try:
...
def update_existing_invoice(invoice_id: int, invoice: InvoiceGeneration, db: Session = Depends(db_session)):
    try:
...
def delete_invoice_by_id(invoice_id: int, db: Session = Depends(db_session)):
    try:
